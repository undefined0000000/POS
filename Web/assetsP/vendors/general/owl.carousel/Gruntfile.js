	...
	grunt
		.initConfig({
			...
			sass: {
				docs: {
					options: {
						...
						includePaths: [ '<%= app.docs.src %>/assetsp/scss/', 'node_modules/foundation-sites/scss' ]
					},
					files: {
						'<%= app.docs.dest %>/assetsp/css/docs.theme.min.css': '<%= app.docs.src %>/assetsp/scss/docs.theme.scss'
					}
				},
				dist: {
					options: {
					...
					files: {
						'dist/assetsp/<%= pkg.name %>.css': 'src/scss/<%= pkg.name %>.scss',
						'dist/assetsp/owl.theme.default.css': 'src/scss/owl.theme.default.scss',
						'dist/assetsp/owl.theme.green.css': 'src/scss/owl.theme.green.scss'
					}
				}
			},
			...
			autoprefixer: {
				...
				dist: {
					files: {
						'dist/assetsp/<%= pkg.name %>.css': 'dist/assetsp/<%= pkg.name %>.css',
						'dist/assetsp/owl.theme.default.css': 'dist/assetsp/owl.theme.default.css',
						'dist/assetsp/owl.theme.green.css': 'dist/assetsp/owl.theme.green.css'
					}
				}
			},
			...
			cssmin: {
				dist: {
					files: {
						'dist/assetsp/<%= pkg.name %>.min.css': 'dist/assetsp/<%= pkg.name %>.css',
						'dist/assetsp/owl.theme.default.min.css': 'dist/assetsp/owl.theme.default.css',
						'dist/assetsp/owl.theme.green.min.css': 'dist/assetsp/owl.theme.green.css'
					}
				}
			},
			...
			copy: {
				...
				distToDocs: {
					...
					cwd: 'dist/',
					src: [ '**/*.*' ],
					dest: '<%= app.docs.dest %>/assetsp/owlcarousel'
				},
				...
				srcToDocs: {
					...
					cwd: 'src/js',
					src: [ '**/*.js' ],
					dest: '<%= app.docs.dest %>/assetsp/owlcarousel/src'
				},

				docsAssets: {
					expand: true,
					cwd: '<%= app.docs.src %>/assetsp/',
					src: [ 'css/*.css', 'vendors/*.js', 'vendors/*.map', 'img/*.*', 'js/*.*' ],
					dest: '<%= app.docs.dest %>/assetsp/'
				},
			...
			watch: {
				...
				sassDocs: {
					files: [ '<%= app.docs.src %>/assetsp/**/*.scss' ],
					tasks: [ 'sass:docs' ]
				},
				sass: {
					files: [ 'src/**/*.scss' ],
